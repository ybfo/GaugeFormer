...
def null_rows(template, rows):
    if len(template) != 5 or any(t.device.type != 'cpu' or t.numel() != 1 for t in template):
...
def install(wrapper):
    ...
    stats = dict(optimization='gtm_constant_granularity_predicate_cache_v1',
                 avoided_scalar_checks=0, metadata_templates=0,
                 tensor_arithmetic_unchanged=True, cuda_graph_used=False)
    cache = {}
    active = None
