...
def standardize_native(context, store):
    if hasattr(store, "normalization_center"):
        ...
        spread_np = np.where(np.asarray(spread_np) == 0, 1., spread_np)
    else:
        center_np, spread_np = native_location_scale(str(store.root.resolve()))
    center = torch.as_tensor(center_np, device=context.device, dtype=context.dtype).view(1, 1, -1)
    spread = torch.as_tensor(spread_np, device=context.device, dtype=context.dtype).view(1, 1, -1)
    return (context-center)/spread, center, spread

def official_module(method):
    path = HERE / "vendor" / method.upper() / ("TEFN.py" if method == "tefn" else "base_model.py")
...
class SchemaBaseline(nn.Module):
    def __init__(self, method, width=WIDTH):
        ...
        if method == "tefn":
            self.core = source.Model(SimpleNamespace(
                task_name="long_term_forecast", seq_len=96, label_len=48, pred_len=24,
                enc_in=width, e_layers=0, dropout=0.1, use_norm=True,
                use_T_model=True, use_C_model=True, fusion_method="add",
                use_probabilistic_layer=False, kernel_activation=None, use_residual=True))
        elif method == "tggc":
            self.core = source.Model(units=width, stack_cnt=2, time_step=96,
                multi_layer=5, horizon=24, dropout_rate=0.4, leaky_rate=0.02,
                coe_a=1.2, coe_b=1., order=4, gconv="gegen", non_linear="linear",
                Fouropt="FB", attention_set="linear", modes=5, activation="softmax", device="cpu")
            ...
            for block in self.core.stock_block:
                block.Fourier.register_buffer("saved_frequency_indices",
    ...
    def load_state_dict(self, state, strict=True, **kwargs):
        ...
        if self.method == "tggc":
            for block in self.core.stock_block:
                block.Fourier.index = block.Fourier.saved_frequency_indices.cpu().tolist()
        return result
    ...
    def pack(self, values, generator=None):
        ...
        slots = np.arange(c) if generator is None else generator.permutation(self.width)[:c]
    ...
    def forward_scaled(self, scaled, generator=None):
        ...
        return forecast[..., slots], None if reconstruction is None else reconstruction[..., slots]
    ...
    def predict_queries(self, x, store):
        ...
        if self.method == "tggc" and len(x) > 1:
            ...
            pred = torch.cat([self.forward_scaled(row[None])[0] for row in scaled], dim=0)
        else:
