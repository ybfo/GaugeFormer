...
def fit(
    ...
    *,
    ...
    device = device_setup()
    output = (
        Path(output) if output else train_path(method, family, seed, target, fraction)
    )
    if completed(output / "complete.json"):
        return output
    if max_windows != 100000 and family != "fewshot" and not smoke:
        raise ValueError(
            "Full source training must use the frozen 100000-window ceiling"
        )
    seed_all(seed)
    ...
    if source:
        source = Path(source)
        if source.is_dir() and not completed(source / "complete.json"):
            raise ValueError("Source training is incomplete")
        source_file = source / "best.pt" if source.is_dir() else source
        state = torch.load(source_file, map_location="cpu", weights_only=True)
        if (
            state["method"] != method
            or state["seed"] != seed
            or target in state["source_systems"]
        ):
            raise ValueError(
                "Few-shot source checkpoint identity or LOSO isolation differs"
            )
        model.load_state_dict(state["model"])
        binding = artifact(source_file)
    model.to(device)
    training_systems = (
        (target,) if family == "fewshot" else tuple(s for s in SYSTEMS if s != target)
    )
    train_stores = {
        s: make_store(s, "train", family, training=True) for s in training_systems
    }
    dev_stores = (
        {}
        if family == "fewshot"
        else {s: make_store(s, "dev", family, training=True) for s in training_systems}
    )
    subsets = {}
    if family == "fewshot":
        if source is None or fraction not in (0.01, 0.05):
            raise ValueError(
                "Few-shot fitting requires its LOSO source and fixed fraction"
            )
        subsets[target] = hashed_subset(
            len(train_stores[target]), fraction, target, "gf-cs-2026-08-20-v7"
        )
    ...
    while windows < max_windows:
        ...
        indices = (
            rng.choice(subsets[system], size=size, replace=True)
        ...
        if time.time() - last_progress >= 30:
            ...
            print(
                f"TRAIN {output.name} {windows}/{max_windows} loss={float(loss.detach()):.6f}",
                flush=True,
            )
        ...
        if family == "fewshot":
            ...
            score, metrics = validate(
                model, dev_stores, device, limit=32 if smoke else 2048
            )
            history.append(
                dict(windows=windows, source_dev_sd_nmae=score, systems=metrics)
            )
            print(
                f"VALIDATE {output.name} windows={windows} source_dev={score:.6f}",
                flush=True,
            )
            if score < best:
    ...
    state = dict(
        ...
        fraction=fraction,
        model=best_state,
        source_systems=list(training_systems),
        ...
        schema_width=26,
        random_slot_training=True,
        supervised_queries={
            s: st.query_indices.tolist() for s, st in train_stores.items()
        },
        fewshot_subsets={
            s: dict(count=len(v), sha256=subset_sha256(v)) for s, v in subsets.items()
        },
    )
    ...
    write(
        ...
        dict(
            ...
            selected_windows=best_windows,
            source_dev_sd_nmae=None if family == "fewshot" else best,
            learning_rate=learning_rate,
            source_systems=list(training_systems),
            test_access=False,
            source_checkpoint=binding,
            artifacts=[artifact(output / "best.pt")],
            elapsed_seconds=time.time() - start,
        ),
    )
        ...
        for windows in (1000, 5000):
            ...
            for target in SYSTEMS:
                ...
                fit(
                    ...
                    fraction,
                    max_windows=windows,
                    lr=BASE_LR[method] * multiplier,
                    source=train_path(method, "loso", 17, target),
                    output=path,
                )
                ...
                if score_path.exists():
                    ...
                    model = SchemaBaseline(method)
                    state = torch.load(
                        path / "best.pt", map_location="cpu", weights_only=True
                    )
                    model.load_state_dict(state["model"])
                    model.to(device)
                    score, values = validate(
                        model, {target: make_store(target, "dev", "fewshot")}, device
                    )
                    metric = dict(
                        sd_nmae=score,
                        metrics=values,
                        checkpoint=artifact(path / "best.pt"),
                    )
...
def main():
    ...
    p.add_argument(
        "--family",
        choices=("joint", "loso", "unseen_target", "fewshot", "calibrate"),
        required=True,
    )
    ...
    p.add_argument(
        "--source-checkpoint",
        type=Path,
        help="Optional fitted LOSO checkpoint for few-shot training",
    )
    p.add_argument("--smoke", action="store_true")
    args = p.parse_args()
    if args.family in ("loso", "fewshot") and args.target is None:
        p.error("LOSO and few-shot fitting require --target")
    if args.family in ("fewshot", "calibrate") and args.fraction not in (0.01, 0.05):
        p.error("Few-shot fitting and calibration require --fraction .01 or .05")
    if args.source_checkpoint and args.family != "fewshot":
        p.error("--source-checkpoint is used only for few-shot fitting")
    if args.family == "calibrate":
        calibrate(args.method, args.fraction)
    elif args.family == "fewshot":
        selections = read(PROJECT / "configs/fitting.json")
        selection = next(
            r
            for r in selections
            if r["method"] == args.method and r["fraction"] == args.fraction
        )
        source = args.source_checkpoint or train_path(
            args.method, "loso", args.seed, args.target
        )
        if not source.exists():
            source, _ = checkpoint_record(args.method, "loso", args.seed, args.target)
        fit(
            args.method,
            args.family,
            args.seed,
            args.target,
            args.fraction,
            max_windows=selection["fine_tuning_windows"],
            lr=BASE_LR[args.method] * selection["learning_rate_multiplier"],
            source=source,
        )
    else:
        fit(
            ...
            args.target,
            args.fraction,
            max_windows=64 if args.smoke else 100000,
            smoke=args.smoke,
            output=(
                BASELINE_ROOT / "smoke" / f"train_{args.method}" if args.smoke else None
            ),
        )
