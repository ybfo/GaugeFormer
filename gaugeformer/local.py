...
def damped_holt(
    ...
    *,
    ...
    for index in range(1, values.shape[1]):
        previous = level
        level = alpha * values[:, index] + (1.0 - alpha) * (level + damping * trend)
...
def expert_predictions(context, query, *, horizon=24):
    ...
    stable = np.stack(
        (
            _persistence(values, horizon),
            _damped_trend(values, horizon, 12, 0.8),
            _damped_trend(values, horizon, 24, 0.9),
        )
    )[:, query]
    levels = np.stack(
        [recent_median(values, horizon, w) for w in (6, 12, 24)]
        + [
            damped_holt(values, horizon, alpha=0.3, beta=0.1, damping=rho)
            for rho in (0.8, 0.95)
        ]
...
def selected_experts(contexts, query):
    ...
    _, predictions = expert_predictions(
        values.reshape(count * channels, length), np.arange(count * channels)
    )
    return predictions.reshape(8, count, channels, 24).transpose(1, 0, 2, 3)[
        :, :, query
    ]
