...
class GaugeDynamicsMemory:
    ...
    def update_and_predict(
        ...
        if candidates.ndim != 3:
            raise ValueError(
                "future_candidates must have shape [experts, query, horizon]"
            )
        if losses.ndim != 3 or losses.shape[1:] != candidates.shape[:2]:
            raise ValueError(
                "backcast_losses must have shape [origins, experts, query]"
            )
        if losses.shape[0] < 1:
        ...
        if self.cumulative_losses.shape != candidates.shape[:2]:
            raise ValueError(
                "candidate or query schema changed inside an online stream"
            )

        self.cumulative_losses += losses.sum(axis=0)
        self.cumulative_origin_wins += (losses < losses[:, 0:1, :]).sum(axis=0)
    ...
    def evidence_summary(self, candidate_names: tuple[str, ...]) -> dict[str, object]:
        ...
        return {
            ...
            "origin_win_fraction_vs_raw": {
                name: float(
                    np.mean(
                        self.cumulative_origin_wins[index] / self.observed_origin_panels
                    )
                )
                for index, name in enumerate(candidate_names)
            },
