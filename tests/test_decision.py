...
def test_warmup_and_exact_backbone_fallback():
    memory = GaugeDynamicsMemory(OnlineRouterConfig(blend=.5))
    candidates = np.array([[[10., 10.]], [[2., 2.]]])
    losses = np.array([[[5.], [1.]]]*4)
    for _ in range(15):
    ...
    pred, choice, eligible = memory.update_and_predict(candidates, losses)
    np.testing.assert_array_equal(pred, [[6.,6.]])
    assert eligible.all() and choice[0] == 1

def test_mean_advantage_and_repeated_wins_are_separate():
    memory = GaugeDynamicsMemory(OnlineRouterConfig(blend=.5,warmup_windows=1))
    candidates = np.array([[[3.]], [[1.]]])
    losses = np.array([[[10.],[0.]],[[1.],[2.]],[[1.],[2.]],[[1.],[2.]]])
...
def test_historical_predictions_see_only_their_prefix():
    ...
    q=np.array([0,2]);historical=np.zeros((4,2,24),np.float32)
    first=context_backcast_losses(x,q,historical)
    changed=x.copy();changed[:,80:]+=100
...
def test_positive_affine_and_permutation_consistency():
    ...
    x=rng.normal(size=(4,96));q=np.array([0,2,3]);a=2.;c=5.
    ...
    np.testing.assert_allclose(transformed.prediction,a*out.prediction+c,atol=2e-6)
    perm=np.array([2,0,3,1]);new_q=np.argsort(perm)[q]
...
def test_rejects_invalid_query_schema():
    with pytest.raises(ValueError,match='unique'):
        GaugeFormer().predict(np.zeros((2,96)),np.array([0,0]),np.zeros((2,24)),np.zeros((4,2,24)))
