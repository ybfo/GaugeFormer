...
class SundialModel(SundialPreTrainedModel):
    ...
    def forward(
        ...
        # print(attention_mask.size(), batch_size, seq_length, inputs_embeds.size()) # torch.Size([64, 4096]) 64 256 torch.Size([64, 256, 768])
...
class SundialForPrediction(SundialPreTrainedModel, TSGenerationMixin):
    ...
    def forward(
        ...
        loss = None
        mae, mse, trend_diff = None, None, None
        if labels is not None:
            # print(hidden_states.size()) # torch.Size([64, 256, 768])
            if revin:
            ...
            labels = labels[:, :seq_len -
                            self.config.input_token_len + output_token_len] # 4096-16+720
            shift_labels = labels.unfold(
                dimension=-1, size=output_token_len, step=self.config.input_token_len)

            if not training: # 非训练、验证的时候算吧,太慢了
                mae, mse, trend_diff = self.flow_loss.meterics(labels[:, -output_token_len:], hidden_states[:, -1, :], loss_masks[:, -(output_token_len//self.config.input_token_len):] if loss_masks is not None else None)

            bsz, L, _ = shift_labels.shape
            # print(seq_len, hidden_states.size(), labels.size(), shift_labels.shape, loss_masks.shape) # L=1?
            # 4096 torch.Size([64, 256, 768]) torch.Size([64, 4800]) torch.Size([64, 256, 720]) torch.Size([64, 300])
            shift_labels = shift_labels.reshape(
                bsz * L, -1).repeat(self.config.diffusion_batch_mul, 1) 
            hidden_states = hidden_states.reshape(
                bsz * L, -1).repeat(self.config.diffusion_batch_mul, 1)
            loss_masks = loss_masks.reshape(
                bsz * L).repeat(self.config.diffusion_batch_mul) if loss_masks is not None else None
            mask_y = mask_y.repeat(L * self.config.diffusion_batch_mul, 1) if mask_y is not None else None

            # print(hidden_states.size(), shift_labels.shape, loss_masks.shape) # L=1?
